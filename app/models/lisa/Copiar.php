 ...
 * Modelo Copiar
 */
class Copiar extends Models implements IModels
{
    ...
    public function updateFacturacionPedido()
    {
        ...
        $this->postDataAllFact($doc['dataFacturacion']['examenesFact']);
    ...
    public function postDataAllFact($datosFacturados)
    {
    ...
    public function getCopy()
    {
        ...
        try {
            ...
            $_list = Helper\Files::get_files_in_dir($this->dirIngresados, '.xml');
            ...
            foreach ($list as $key => $val) {
                ...
                if ($i <= 10000) {
                    ...
                    if (date('Y-m-d', strtotime($fechaPedido)) == '2024-08-06') {
                        ...
                        // file_put_contents('../v1/lisa/pedidos/v2/ingrea/' . basename($val), $xml);
                    }
                }
    ...
    public function getCopyEnviados()
    {
        ...
        try {
            ...
            $_list = Helper\Files::get_files_in_dir($this->dirRetenidos . 'sector/hospitalizacion/', '.xml');
            ...
            foreach ($list as $key => $val) {
                ...
                if ($i <= 10000) {
                    ...
                    if (date('Y-m-d', filemtime($val)) == '2024-08-12') {
                        $ordenes[] = array(
                            'fecha' => date("d-m-Y H:i:s", filemtime($val)),
                            'val' => $val,
                        );

                        file_put_contents('../v1/lisa/pedidos/v2/retenidos/sector/hospitalizacion/' . basename($val), $xml);
                    }
                }
                ...
                if ($typeFilter == 1) {
                    ...
                    if ($i <= 1000) {
                        ...
                        if ($typeFilter == 3) {
                            ...
                            if ((strpos($sector, 'HOSPITALIZACION') !== false || strpos($sector, 'UCI') !== false) && (strtotime(date('Y-m-d', strtotime($fechaPedido))) >= $timedesde) && (strtotime(date('Y-m-d', strtotime($fechaPedido))) <= $timeHasta)) {
                                $ordenes[] = array(
                                    'codigoPedido' => (isset($_codigoPedido[0]) ? $_codigoPedido[0] : ''),
                                    'fechaPedido' => (isset($_fechaPedido[0]) ? $_fechaPedido[0] : ''),
                                    'at_mv' => (isset($_codigoAtendimento[0]) ? $_codigoAtendimento[0] : ''),
    ...
    public function getIngresadasFlebotomista_v3($toma = '')
    {
        ...
        try {
            ...
            foreach ($data as $key) {
                ...
                if ($stsCall == 0 && $stsPen == 0 && isset($jsonLlamado['toma']) && $jsonLlamado['toma'] == $toma) {
                    $ordenes[] = array(
                        'cdAtendimento' => $key['CD_ATENDIMENTO'],
                        'codigoPedido' => $key['SC'],
